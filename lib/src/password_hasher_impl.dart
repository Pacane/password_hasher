...
class PasswordHasher {
  ...
  PasswordHasher(
      ...
      int keyLength: 32}) {
    this.saltGenerator =
        saltGenerator == null ? new RandomSaltGenerator() : saltGenerator;
  ...
  bool checkPassword(String saltAndPasswordHash, String password) {
    ...
    var storedPasswordHash =
        SecurityHelper.extractPasswordHashFromHash(saltAndPasswordHash);
    var newHash = SecurityHelper.hashPasswordAndSalt(
        salt, password, hash, _iterations, _keyLength);
...
class SecurityHelper {
  ...
  static List<int> hashPasswordAndSalt(List<int> salt, String password,
      Hash hash, int iterations, int keyLength) {
    var gen = new PBKDF2(hash: hash);
    return gen.generateKey(
        password, new String.fromCharCodes(salt), iterations, keyLength);
  }
